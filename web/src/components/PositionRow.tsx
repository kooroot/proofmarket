...
export function PositionRow({ m, pos }: { m: UiMarket; pos: { yesAmount: bigint; noAmount: bigint; claimed: boolean } }) {
  ...
  const [sig, setSig] = useState<string | null>(null); const [txErr, setTxErr] = useState<string | null>(null); const [busy, setBusy] = useState(false);
  ...
  return (
    <div className="flex items-center justify-between border-b border-zinc-800 py-2 text-sm">
      ...
      <div className="flex gap-2">
        ...
        <Button size="sm" disabled={!wallet || !claimable(m, pos) || busy} onClick={async () => {
        ...
        }}>{busy ? "Confirming…" : pos.claimed ? "Claimed" : "Claim"}</Button>
      </div>
