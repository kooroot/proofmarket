...
export function HashChip({ bytes, tone, prefix }: { bytes: number[]; tone: "sky" | "violet" | "fuchsia" | "emerald"; prefix?: string }) {
  ...
  return (
    <span className={`inline-flex max-w-full items-center gap-1 overflow-hidden rounded-md border px-1.5 py-0.5 align-middle font-mono text-[11px] ${tones[tone]}`}>
  ...
  return (
    <>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.24 }}
        ...
        <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border text-xs font-semibold sm:h-7 sm:w-7 ${
      ...
      <motion.div data-step initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.24 }}
        className={`min-w-0 overflow-hidden rounded-lg border bg-zinc-900/60 p-3 space-y-1.5 sm:p-4 ${green ? "border-emerald-500/60" : "border-zinc-800"}`}>
        <div className="font-medium leading-tight">{title}</div>
        <div className="text-xs text-zinc-400">{subtitle}</div>
        {body && <div className="min-w-0 overflow-x-auto font-mono text-xs leading-relaxed text-zinc-300 [-webkit-overflow-scrolling:touch]">{body}</div>}
        {link && <a className="break-words text-xs text-emerald-400" href={link} target="_blank" rel="noreferrer">{linkLabel} →</a>}
        <div className="break-all text-[10px] text-zinc-600">source: {source}</div>
      </motion.div>
    </>
  );
}
  ...
  return (
    <>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.24 + 0.2 }}
        className="flex min-w-0 items-center overflow-x-auto whitespace-nowrap py-1.5 font-mono text-[11px] text-zinc-500 [-webkit-overflow-scrolling:touch]">
